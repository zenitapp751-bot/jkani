...
const JKAnimeScraper = (function () {
    ...
    async function resolveEpisodeStream(episodeUrl) {
        try {
            const html = await proxyFetch(episodeUrl);
            const parser = new DOMParser();
            const doc = parser.parseFromString(html, 'text/html');
            ...
            iframeElements.forEach(el => {
                ...
                if (src) {
                    ...
                    else if (src.startsWith('/')) src = 'https://jkanime.net' + src;
                    if (src.includes('jkplayer') || src.includes('jkokru') || src.includes('desu') || src.includes('nozomi') || src.includes('c1') || src.includes('um')) {
                        iframesRaw.push(src);
                    }
                }
            });
            ...
            if (matchesRegex) {
                matchesRegex.forEach(u => {
                    if (!iframesRaw.includes(u)) iframesRaw.push(u);
                });
            }
            ...
            for (const iframeUrl of iframesRaw) {
                try {
                    ...
                    if (m3u8Match && m3u8Match[1]) {
                        if (!streamResults.m3u8_streams.includes(m3u8Match[1])) {
                        ...
                        if (b64Matches) {
                            b64Matches.forEach(b64 => {
                                const clean = b64.replace(/atob\s*\(\s*["']/, '').replace(/["']\)/, '');
                                try {
                                    const dec = atob(clean);
                                    if (dec.includes('.m3u8')) {
                                        const found = dec.match(/(https?:\/\/[^\s"']+\.m3u8[^\s"']*)/i);
                                        if (found && !streamResults.m3u8_streams.includes(found[1])) {
                                            streamResults.m3u8_streams.push(found[1]);
                                        }
                                    }
                                } catch (e) {}
                            });
            ...
            return { success: true, data: streamResults };
        } catch (e) {
            return { success: false, error: e.message, data: { iframes: [], m3u8_streams: [] } };
        }
    }
